            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                {profiles?.map(profile => (
                    <Link href={`/profiles/${profile.id}`} key={profile.id} className="flex flex-col justify-center gap-2 items-center p-1">
                        <img className="object-cover rounded-full w-35 h-35 shadow-md hover:shadow-xl" width={150} height={150} src={profile.avatar_url || "https://placehold.co/600x600/png?text=Item"} alt={`${profile.display_name}'s profile avatar`} />
